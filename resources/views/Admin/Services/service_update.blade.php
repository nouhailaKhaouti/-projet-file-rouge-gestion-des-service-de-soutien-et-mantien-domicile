    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="example{{$loop->iteration }}ModalLabel">update {{$service->libelle}}</h5>
                <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="{{url('service_update',$service->id)}}" method="post">
                    @csrf
                    <input type="text" name="libelle" value="{{$service->libelle}}" class="form-control">
